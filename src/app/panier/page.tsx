...
export default async function CartPage() {
  const [cart, shippingZones, addresses] = await Promise.all([getCart(), getShippingZones(), getMyAddresses()]);

  return (
    <div className="mx-auto max-w-6xl px-6 md:px-8 py-16">
      <h1 className="section-title">MON PANIER</h1>
      <div className="section-title-underline" />
      <CartClient initialCart={cart} shippingZones={shippingZones} addresses={addresses} />
    </div>
  );
}
